...
@fieldwise_init
struct Player(Movable, ImplicitlyCopyable):
    var glyph: Codepoint
...
@fieldwise_init
struct Coordinate(ImplicitlyCopyable, Hashable, Equatable, Writable):
    var row: Int
    var col: Int
    ...
    def write_repr_to(self, mut writer: Some[Writer]):
        writer.write(
            t"Coor(row:{self.row}, col:{self.col})"
        )
...
struct GameState:
    ...
    def __init__(out self):
        ...
        self.players = [Player(Codepoint.ord('X'),1), Player(Codepoint.ord('O'),2)]
...
struct GameRenderer(Movable):
    ...
    def get_right_rect(self, input_rect: Rect) -> Rect:
        var w_l = input_rect.width//2
        return Rect(input_rect.row,input_rect.col+w_l+1,w_l,input_rect.height)
    ...
    def draw_board_rect(mut self):
        ...
        for row_idx in range(3):
            ...
            for col_idx in range(3):
                ...
                # except:
                #     pass
                self.state_map[Coordinate(row_idx,col_idx)] = Coordinate(row_start+row_offset,col_start+col_offset)
    ...
    def draw_game_state(self, game_state : GameState):
        ...
        for item in self.state_map.items():
            ...
            try:
                self.p_win[].write_char(item.value.row,item.value.col,codepoint)
            except e:
                print(e)
        
        # Update the cursor:
        self.place_cursor(game_state.cursor_cell.row,game_state.cursor_cell.col)
